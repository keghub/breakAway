...
const sites = new Array(numberOfSites).fill({}).map(() => createSite());

const siteDocs = sites.map((s) => createSiteDoc(s));

const products = new Array(numberOfProducts).fill({}).map(() => {
  const productSiteIds = faker.helpers
    .arrayElements(sites)
    .map((s: ISite) => s.id);
